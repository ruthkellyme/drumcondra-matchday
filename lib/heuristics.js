...
// `icon` is a semantic key, not an emoji — the client renders it as a
// minimal line icon (a filled-bars gauge, more bars = bigger crowd).
const TIER_FLAVOUR = [
  { icon: 'tier-1', tagline: "Low crowd — ah go on, you'll be grand." },
  { icon: 'tier-2', tagline: 'Middling crowd — a small bit of planning goes a long way.' },
  { icon: 'tier-3', tagline: "Big crowd, this one — plan the day around it or you'll be caught out." },
  { icon: 'tier-4', tagline: 'Full house — careful now.' },
];
...
// One human, whole-day overview per event, always the same two-part shape
// (do X before Y, clear of the crowd by Z) — the mid-day detail (exactly how
// much freedom there is between kick-off and full-time) is already on the
// timeline chart right below this, so the callout doesn't need to repeat it.
...
// `icon` is a semantic key — the client renders it as a minimal line icon.
// Deliberately no overlapping wording between these — two both mentioning
// "the messages" made back-to-back days read as repetitive even when the
// random pick was technically working correctly.
const PREP_ACTIONS = [
  { icon: 'car', verb: 'Move the car' },
  { icon: 'dog', verb: 'Walk the dog' },
  { icon: 'cart', verb: 'Get the messages done' },
];

function buildTip({ tierIndex, roadClosureMin, finalWhistleLate, roadFullyClearMin, parkingFullyClearMin, footTaperEnd, firstKickoff, seed }) {
  const flavour = TIER_FLAVOUR[tierIndex];

  if (tierIndex === 0) {
    return { flavour, tip: { min: firstKickoff, icon: 'check', text: "Grand and quiet, this one — go on, go on, go on, no need to change the plans." } };
  }

  const action = seededPick(PREP_ACTIONS, seed);
  const allClear = Math.max(roadFullyClearMin, parkingFullyClearMin, footTaperEnd);

  let text = `${action.verb} before ${formatClock12(roadClosureMin)} — you're grand again once it clears by about ${formatClock12(allClear)}.`;
  if (tierIndex === 3) {
    text += ' Full house today, so the hold at the Garda cordon after could run on a fair while.';
  }
...
export function applyHeuristics(event) {
  ...
  const roadsLane = toSegments([
    ...
    { atMin: postMatchSurgeStart, status: 'critical', label: 'Post-match surge — no exceptions, even with a pass', official: false },
