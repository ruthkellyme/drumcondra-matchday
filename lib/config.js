...
// Confirmed manually: this is exactly where Clonliffe Road, Ballybough Road,
// Fairview Strand and Poplar Row all meet. Hardcoded rather than derived
// live because Nominatim's data here is unreliable between requests — the
// long road's own fetch sometimes drops a segment short of this point, and
// Poplar Row itself has a second, disconnected OSM way sharing its name that
// sits closer to the stadium's centre point than the real junction does, so
// no distance-based heuristic reliably lands here every time.
export const CLONLIFFE_ROAD_EAST_JUNCTION = [53.3613725, -6.2427661];

// Physical Garda barrier points reported by a resident — also not on Croke
// Park's own notice. Each is either where two streets meet ("junction") or
// the far ends of one road ("end-fixed", anchored to a known coordinate
// rather than derived, for roads where live geocoding proved unreliable).
export const RESIDENT_REPORTED_CORDON_POINTS = [
  { label: 'Fitzgibbon Street & Russell Street', kind: 'junction', roads: ['Fitzgibbon Street', 'Russell Street'] },
  { label: 'Clonliffe Road', kind: 'end-fixed', road: 'Clonliffe Road', point: CLONLIFFE_ROAD_EAST_JUNCTION },
];

// Croke Park's own notice lists these as access roads, but a resident
// reports they're actually cordoned off same as the official closure roads
// — shown as part of the closed zone instead of a resident entrance.
export const RESIDENT_REPORTED_CLOSURES = ['Ballybough Road'];

// Where "nearest to the closure roads" picks the wrong end of an access
// road, a resident can instead name the real cross-street the entrance
// actually sits at.
export const RESIDENT_REPORTED_ENTRANCE_ANCHORS = {
  'Fitzroy Avenue': 'Drumcondra Road Lower',
  'Clonliffe Avenue': 'Ballybough Road',
};
