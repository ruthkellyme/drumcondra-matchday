...
// Croke Park has used both "full house" and "full stadium" across different
// notices for the same thing — treat either as a confirmed sell-out.
function looksFullHouse(str) {
  return /full house|full stadium/i.test(str);
}
...
function parseAttendanceFromLines(lines) {
  for (const line of lines) {
    ...
    if (!m) continue;
    if (looksFullHouse(m[1])) return { isFullHouse: true, value: CROKE_PARK_CAPACITY };
    const num = m[1].match(/(\d{1,3}(?:,\d{3})+|\d{4,6})/);
    if (num) return { isFullHouse: false, value: parseInt(num[1].replace(/,/g, ''), 10) };
  }
  // Some notices only say it as prose ("With a full stadium at Croke Park
  // this weekend...") with no "Attendance:" line at all — a bare full-house
  // mention anywhere in the day's own block is unambiguous enough to accept.
  for (const line of lines) {
    if (looksFullHouse(line)) return { isFullHouse: true, value: CROKE_PARK_CAPACITY };
  }
  return null;
}
...
export async function parseEventsFromHtml(html, { now = new Date() } = {}) {
  ...
  for (const [key, block] of fixtureBlocks) {
    ...
    const restrictionParsedOk = restriction && (restriction.parkingRestrictionTime || restriction.roadClosureTime);
    const needsReview = fixtures.length === 0 || !attendance || !restrictionParsedOk;
    if (needsReview) {
      const label = `${block.dayName} ${block.month} ${block.day}`;
      // Only the kick-off time is missing and everything else parsed fine —
      // that's Croke Park not having published it yet, not a scraper failure,
      // so say so rather than implying something on our end went wrong.
      if (fixtures.length === 0 && attendance && restrictionParsedOk) {
        warnings.push(`Croke Park hasn't published kick-off times for "${label}" yet — check ${SOURCE_URL} closer to the date, or use manual entry to add one.`);
      } else {
        warnings.push(`"${label}" parsed incompletely — check it against ${SOURCE_URL} or use manual entry to correct it.`);
      }
    }
